...
def walkOnlyOneLevel(path):
    dirRes = []
    fileRes =[]
    for root,dirs,files in os.walk(path, topdown=True):
        depth = root[len(path) + len(os.path.sep):].count(os.path.sep)
        if depth <= 1:
            dirRes += [os.path.join(root, d) for d in dirs]
            fileRes += [os.path.join(root, f) for f in files]
            dirs[:] = [] # Don't recurse any deeper
    return dirRes, fileRes
...
class FileTree():
    ...
    def list_files(partition):
        dirRes = []
        fileRes =[]
        for root,dirs,files in os.walk(partition, topdown=True):
            depth = root[len(partition) + len(os.path.sep):].count(os.path.sep)
            if depth <= 1:
                dirRes += [os.path.join(root, d) for d in dirs]
                fileRes += [os.path.join(root, f) for f in files]
                dirs[:] = [] # Don't recurse any deeper
...
ft = FileTree(Tk())
paths = [FileTree.list_files(path) for path in ["C:\\", "E:\\"]]
data = {"C:\\": paths[0], "E:\\": paths[1]}
ft.data = data
ft.startInstance()
print(paths)